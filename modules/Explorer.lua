...
local function main()
	...
	Explorer.InitRightClick = function()
		...
		context:Register("SAVE_INST",{Name = "Save Model (.rbxm)", IconMap = Explorer.MiscIcons, Icon = "Save", OnClick = function()
			local sList = selection.List
			if #sList == 0 then return end

			local saveinstance = env.saveinstance
			if not saveinstance then
				warn("[Dex] Save Model: executor does not expose saveinstance.")
				return
			end
			...
			if #sList == 1 then
				-- Single selection: clone langsung, hierarki children ikut otomatis
				local s, cloned = pcall(game.Clone, sList[1].Obj)
				if not s or not cloned then
					warn("[Dex] Save Model: failed to clone instance.")
					return
				end
				cloned.Parent = nil -- detach dari game tree sebelum save
				container = cloned
				needsCleanup = true
			else
				-- Multi-selection: hanya clone instance yang TIDAK punya ancestor
				-- dalam selection (untuk menghindari duplikasi children)
				local selectionSet = {}
				for i = 1, #sList do
					selectionSet[sList[i].Obj] = true
				end

				-- Cek apakah sebuah instance punya ancestor dalam selection
				local function hasSelectedAncestor(obj)
					local parent = obj.Parent
					while parent and parent ~= game do
						if selectionSet[parent] then return true end
				...
				container.Name = "DexSavedSelection"
				needsCleanup = true

				for i = 1, #sList do
					local obj = sList[i].Obj
					-- Skip jika parent-nya sudah ada di selection
					-- (sudah ikut ter-clone sebagai child)
					if not hasSelectedAncestor(obj) then
						local s, cloned = pcall(game.Clone, obj)
						if s and cloned then
							-- Preserve posisi dalam hierarki relatif:
							-- clone subtree-nya langsung ke container
				...
				if #container:GetChildren() == 0 then
					container:Destroy()
					warn("[Dex] Save Model: tidak ada instance yang bisa di-clone.")
					return
				end
			end

			-- Generate filename
			local instName = tostring(
				(#sList == 1 and sList[1].Obj.Name) or "DexSavedSelection"
			):gsub("[^%w_%-]", "_")
			local timestamp = tostring(math.floor(tick())):sub(-6)
			local fileName = "dex/saved/" .. instName .. "_" .. timestamp .. ".rbxm"

			local success, err = pcall(saveinstance, container, fileName)

			if success then
				print("[Dex] Model saved to: " .. fileName)
			else
				warn("[Dex] Save Model failed: " .. tostring(err))
			end

			if needsCleanup and container then
...
return search]==]
	...
	Explorer.InitSaveMap = function()
		local saveMapBtn = Explorer.GuiElems.ToolBar:FindFirstChild("SaveMap")
		if not saveMapBtn then return end

		-- Tooltip hover label
		local tipLabel = Instance.new("TextLabel")
		tipLabel.Name = "SaveMapTip"
		tipLabel.BackgroundColor3 = Color3.fromRGB(40,40,40)
		tipLabel.BorderSizePixel = 0
		tipLabel.TextColor3 = Color3.new(1,1,1)
		tipLabel.Font = Enum.Font.SourceSans
		tipLabel.TextSize = 12
		tipLabel.Text = "Save Full Map (.rbxl)"
		tipLabel.Size = UDim2.new(0, 130, 0, 18)
		tipLabel.Visible = false
		tipLabel.ZIndex = 10
		tipLabel.Parent = Explorer.GuiElems.ToolBar

		saveMapBtn.MouseEnter:Connect(function()
			local btnPos = saveMapBtn.AbsolutePosition
			local barPos = Explorer.GuiElems.ToolBar.AbsolutePosition
			tipLabel.Position = UDim2.new(0, btnPos.X - barPos.X - 112, 0, 22)
			tipLabel.Visible = true
		end)
		saveMapBtn.MouseLeave:Connect(function()
			tipLabel.Visible = false
		end)
		...
		saveMapBtn.MouseButton1Click:Connect(function()
			...
			local saveinstance = env.saveinstance
			if not saveinstance then
				warn("[Dex] Save Map: executor does not expose saveinstance.")
				return
			end
			...
			local timestamp = tostring(math.floor(tick())):sub(-8)
			local fileName = "dex/saved/map_" .. timestamp .. ".rbxl"

			print("[Dex] Saving full map to: " .. fileName .. " ...")

			local success, err = pcall(saveinstance, game, fileName)
			if success then
				print("[Dex] Full map saved to: " .. fileName)
			else
				warn("[Dex] Save Map failed: " .. tostring(err))
			end
		end)
	end
	...
	Explorer.Init = function()
		...
		local explorerItems = create({
			...
			{3,"Frame",{BackgroundColor3=Color3.new(0.14901961386204,0.14901961386204,0.14901961386204),BorderColor3=Color3.new(0.1176470592618,0.1176470592618,0.1176470592618),BorderSizePixel=0,Name="SearchFrame",Parent={2},Position=UDim2.new(0,3,0,1),Size=UDim2.new(1,-28,0,18),}},
			-- SaveMap button (disk icon, far right of toolbar)
			{12,"TextButton",{AutoButtonColor=false,BackgroundColor3=Color3.new(0.12549020349979,0.12549020349979,0.12549020349979),BackgroundTransparency=1,BorderSizePixel=0,Font=3,Name="SaveMap",Parent={2},Position=UDim2.new(1,-42,0,1),Size=UDim2.new(0,18,0,18),Text="",TextColor3=Color3.new(1,1,1),TextSize=14,}},
			{13,"ImageLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Image="rbxassetid://6031075931",ImageColor3=Color3.new(0.6,0.85,0.6),Parent={12},Position=UDim2.new(0,3,0,3),Size=UDim2.new(0,12,0,12),}},
